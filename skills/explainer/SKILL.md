---
name: explainer
description: "Use when the user says 'make an explainer video', 'explainer deck', 'turn this into a Short, Reel, or TikTok', or runs /explainer <topic>, from a topic or a source document. Generation only; it never posts."
---
...
## Environment
- Package: `~/projects/video-explainer-system` (the user's fork of nemock/video-explainer-system).
- Interpreter: its `uv`-managed venv at `~/projects/video-explainer-system/.venv` (Kokoro, torch,
  Playwright with Chromium, PyMuPDF); `ffmpeg` is the Homebrew one.
- Console commands **`explainer`** and **`deepdive`** are symlinked into `~/.local/bin`. If a shell
  can't find them, use `~/projects/video-explainer-system/.venv/bin/explainer`.
- Brand folder: `~/.claude/explainer-brands/you/` (default `--brand you`).
...
headline/subkicker/url/spoken + optional `product` image. `explainer scaffold … --brand you
